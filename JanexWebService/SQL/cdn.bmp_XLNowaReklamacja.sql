...
alter PROCEDURE cdn.bmp_XLNowaReklamacja
---- dodanie osoby do reklamacji, wersja 2019.3.2
@RlnTyp                 SMALLINT,                       -- typ reklamacji
@DokumentObcy   VARCHAR(40),            -- numer dokumentu reklamacji u kontrahenta
@KntTyp                 INT = NULL,                     -- kontrahent g³ówny
@KntFirma               INT = NULL,                     -- kontrahent g³ówny
@KntNumer               INT,                            -- kontrahent
@KntLp                  SMALLINT,                       -- osoba odbieraj¹ca
@KnATyp                 INT = NULL,                     -- adres kontrahenta g³ównego
@KnANumer               INT = NULL,                     -- adres kontrahenta g³ównego
@KnDTyp                 INT = NULL,                     -- kontrahent docelowy
@KnDFirma               INT = NULL,                     -- kontrahent docelowy
@KnDNumer               INT = NULL,                     -- kontrahent docelowy
@KnDLp                  SMALLINT = NULL,        -- osoba odbieraj¹ca
...
@Miesiac                SMALLINT = NULL,        -- miesi¹c dokumentu
@Seria                  VARCHAR(5) = NULL,      -- seria dokumentu
@Numer                  INT = NULL,                     -- numer dokumentu
@FrsId                  INT = NULL,                     -- Identyfikator centrum wystawiaj¹cego
...
@Wsk            SMALLINT = 0,       -- WskaŸnik do zaznaczania rekordu, wykorzystywane przez API.
...
-- Zwraca numer b³edu:
-- 0 - wykonano pomyœlnie
-- 1 - b³edny typ reklamacji
-- 2 - nie istnieje wskazany kontrahent
-- 3 - nie istnieje wskazany dokument reklamowany lub towar
-- 4 - nie powiod³o siê dodawanie do ReklNag
...
@OpiNumer INT,
@DataMod INT
declare @knstyp int,@knsfirma int,@knsnumer int,@knslp int
...
ELSE
                ...
                where PcK_PicoID = @PicoId),0)
--pobranie osoby
select @knstyp=isnull(KnS_KntTyp,0),
        @knsfirma=isnull(KnS_KntFirma,0),
        @knsnumer=isnull(KnS_KntNumer,0),
        @knslp=isnull(KnS_KntLp,0)
 from cdn.KntOsoby where KnS_KntTyp=@KntTyp and KnS_KntNumer=@KntNumer and KnS_KntLp=@KntLp
...
IF @@ROWCOUNT=0 BEGIN
        RAISERROR ('XLNowaReklamacja: Nie powiod³o siê dodawanie wpisu do tabeli CDN.ReklNag.', 16, 1)
        ROLLBACK TRAN
        RETURN 4
END
...
IF @@ROWCOUNT=0 BEGIN
        RAISERROR ('XLNowaReklamacja: Nie powiod³o siê dodawanie wpisu do tabeli CDN.ReklElem.', 16, 1)
        ROLLBACK TRAN
        RETURN 5
END
...
IF @@ROWCOUNT=0 BEGIN
        RAISERROR ('XLNowaReklamacja: Nie powiod³o siê dodawanie wpisu do tabeli CDN.ReklRealizacja.', 16, 1)
        ROLLBACK TRAN
        RETURN 7
END
