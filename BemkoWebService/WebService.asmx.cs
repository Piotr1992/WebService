...
namespace BemkoWebService
{
    ...
    public class WebService : System.Web.Services.WebService
    {
        ...
        public string b2b_zamelem(int gidnumer, int kndnumer)
        {
            ...
            {
                List<QueryParam> lstPar = new List<QueryParam>();
                lstPar.Add(new QueryParam("@zannumer", gidnumer));
                lstPar.Add(new QueryParam("@kndnumer", kndnumer));
                DataTable dt = DBHelper.RunSqlProcParam("dbo.b2b_sp_zamelem", "b2b_sp_zamelem", lstPar);
                   // .RunSqlQueryParam("select * from [dbo].[b2b_zamelem](@gidnumer,@kndnumer)", "b2b_zamelem", lstPar);
                if (dt != null)
                {
                    return DataTableHelper.GetJson(dt);
                }
        ...
        [WebMethod]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public string b2b_SprzedazKontrahentaSN(int kntnumer, string searchString)
        {
            string message = "";
            if (LogUsage.Params) message = $"Parameters - kntnumer:{kntnumer},searchString:{searchString}";
            using(var w = new UsageLogger("WebSerwis", MethodBase.GetCurrentMethod(), message ?? ""))
            {
                try
                {
                    return WSBReklamacje.B2B_SprzedazKontrahentaSN(kntnumer,searchString);
                }
                catch (Exception ex)
                {
        ...
        [WebMethod]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public string b2b_ReklSprzedazKontrahentaSN(int kntnumer, string searchString)
        {
            string message = "";
            if (LogUsage.Params) message = $"Parameters - kntnumer:{kntnumer},searchString:{searchString}";
            using(var w = new UsageLogger("WebSerwis", MethodBase.GetCurrentMethod(), message ?? ""))
