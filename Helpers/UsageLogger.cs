...
namespace Helpers
{
    public class UsageLogger : IDisposable
    {
        ...
        public UsageLogger(string appName,string typeName,string methodName,string message)
        {
            ...
            _message=message;
            Logger.Logger.LogDebug($"{_typeName}.{_methodName} {_message}");
        }
        ...
        private void CleanUp(object p)
        {
            ...
            _execWatch.Stop();
                if (LogUsage.Usage)
                {
                    ...
                    logWatch.Stop();
                    if(logWatch.ElapsedMilliseconds>100) Logger.Logger.LogWarn($"{_typeName}.{_methodName} LogUsage LogTime:{logWatch.ElapsedMilliseconds}ms");
                }
        }
